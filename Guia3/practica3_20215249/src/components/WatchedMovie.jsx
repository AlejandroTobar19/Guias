...
export function WatchedMoviesList({ watched, onRemoveWatched }) {
    return (
        <ul className="list">
            {watched.map((movie) => (
                <WatchedMovie movie={movie} key={movie.imdbID} onRemoveWatchedn ={onRemoveWatched} />
            ))}
        </ul>
    );
}
export function WatchedMovie({ movie, onRemoveWatchedn }) {
    return (
        <li>
            ...
            <h3>{movie.title}</h3>
                ...
                <button className="btn-delete" onClick={()=> onRemoveWatchedn(movie.imdbID)}>X</button>
            </div>
        </li>
    );
}
